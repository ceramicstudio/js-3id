...
import { rpcError } from '../errors/rpc'
...
export class ConnectService extends IframeService<DIDProviderMethods> {
  ...
  async init(
    ...
    // if exist in state, return before even looking up links
    ...
    let legacyDid: string | null = await legacyDidPromise
    let muportDid
    ...
    try {
      ...
      if (!legacyDid) {
        ...
        did = await manage.createAccount({
          legacyDid,
          skipMigration: Boolean(muportDid || willFail),
        })
      }
    } catch (e) {
      if (legacyDid) {
    ...
    this.provider = this.threeId.getDidProvider(domain)
  ...
  async requestHandlerDid<K extends DIDMethodName>(
    ...
    // TODO: investigate issues with Domain
    // domain?: string
  ): Promise<
  ...
  > {
    ...
    try {
      ...
      const res = await this.provider.send(message /*, domain*/) // TODO: look into what's going on here
  ...
  async _relayDidReq<K extends keyof DIDProviderMethods>(
    ...
    assert.isDefined(this.provider, 'DID provider must be defined')
    return await this.provider.send(req /*, domain*/) // TODO: look into what's going on here
  }
